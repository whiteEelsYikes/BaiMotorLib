...
typedef struct Controller {
    ...
    /* 初始化控制器（可选） */
    void (*init)(struct Controller *self);

    /*
     * 控制器更新函数：
     * 返回控制量（如 PWM、电流、扭矩）
     */
    float (*update)(struct Controller *self);

    /* 获取控制器内部输入状态（调试） */
    int (*get_input)(struct Controller *self);

    /* 获取控制器输出状态（调试） */
    int (*put_output)(struct Controller *self);

    /* 控制器公共参数（如 target） */
    ControllerParams *params;

    /* 控制器内部对象（PID 参数、误差、积分等） */
    void *obj;
    ...
    /* 设置刹车模式 */
    void (*set_brake)(struct Controller *self, MotorBrakeMode mode);
